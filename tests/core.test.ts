...
test('migrations are repeatable, evidence persists, and project deletion cleans artifacts', () => {
  ...
  try {
    ...
    store = new Store(root); assert.equal(store.list<any>('notes')[0].text, 'Resume exactly here'); assert.equal(store.readArtifact(artifact).toString(), '{}'); assert.equal(store.db.prepare('SELECT count(*) AS n FROM migrations').get()!.n, 3);
