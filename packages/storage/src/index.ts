...
export const tables = ['projects', 'environments', 'flows', 'scenarios', 'runs', 'events', 'artifacts', 'findings', 'suppressions', 'notes', 'task_presets', 'repo_snapshots'] as const;
...
export class Store {
  ...
  constructor(root = process.env.DCR_DATA_DIR || resolve('.dcr')) {
    ...
    try {
      ...
      this.db.exec('INSERT OR IGNORE INTO migrations VALUES (1); COMMIT;');
