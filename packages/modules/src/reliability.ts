...
import { fixtureInputSchema, scenarioInputSchema, mutateFixture, type Scenario, type ScenarioDefinition, type ApiFixture, type Matrix } from '../../core/src/reliability.ts';
...
export class ReliabilityCommands {
  ...
  capture(projectId: string, input: unknown) {
    ...
    const url = assertApiTarget(project, data.url).href;
    return this.start(flow, { kind: 'capture', name: `Capture ${data.name}`, version: 1, fixtureName: data.name, url });
  }
  saveFixture(projectId: string, input: unknown, fixtureId?: string) {
    ...
    assertApiTarget(project, parsed.url);
    if (Buffer.byteLength(JSON.stringify(parsed.json)) > 64000) throw new Error('Fixtures must be at most 64 KB');
    ...
    return this.store.put('api_fixtures', { ...parsed, name: redactText(parsed.name), json: redact(parsed.json, project.config.redactFields), id: previous?.id || id(), projectId, version: (previous?.version || 0) + 1, createdAt: previous?.createdAt || now(), sourceRunId: previous?.sourceRunId }) as ApiFixture;
  }
